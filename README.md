...
| `workflows/daily-followup-reminder.json` | Schedule trigger (cron) → find stale leads → send follow-up email |
| `workflows/new-lead-notify.json` | App integration sample: new lead → format message → notify on Slack/WhatsApp |
...
1. In n8n, go to **Workflows → Import from file** and choose one of the JSON files in `workflows/`.
